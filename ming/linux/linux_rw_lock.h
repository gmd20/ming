...
public:
  ...
  void ReadLock()
  {
    (void)::pthread_rwlock_rdlock(&rw_lock_);
  }

  void WriteLock()
  {
    (void)::pthread_rwlock_wrlock(&rw_lock_);
  }

  void Unlock()
  {
    (void)::pthread_rwlock_unlock(&rw_lock_);
  }

  class ReadScopedLock
  {
    public:
      explicit ReadScopedLock(RWLock& lock) : lock_(lock)
      { lock_.ReadLock(); }
      ~ReadScopedLock()
      { lock_.Unlock(); }
    private:
  ...
  {
    public:
      explicit WriteScopedLock(RWLock& lock) : lock_(lock)
      { lock_.WriteLock(); }
      ~WriteScopedLock()
      { lock_.Unlock(); }
    private:
